...
data "hcso_compute_flavors" "flavors" {
  availability_zone = data.hcso_availability_zones.zones.names[1]
...
resource "hcso_waf_dedicated_instance" "instance_1" {
  ...
  available_zone     = data.hcso_availability_zones.zones.names[1]
  ...
  security_group = [
    hcso_networking_secgroup.secgroup.id
  ]
}

resource "hcso_waf_policy" "policy_1" {
  name  = var.waf_policy_name
  level = 1

  depends_on = [
    hcso_waf_dedicated_instance.instance_1]
...
resource "hcso_waf_dedicated_domain" "domain_1" {
  ...
  proxy_id    = hcso_waf_policy.policy_1.id
  keep_policy = true
