...
resource "hcso_vpc_subnet" "vpc_subnet_1" {
...
resource "hcso_dcs_instance" "instance_1" {
  ...
  availability_zones = [
    data.hcso_availability_zones.zones.names[0]
  ]
  vpc_id        = hcso_vpc.vpc_1.id
  subnet_id     = hcso_vpc_subnet.vpc_subnet_1.id
  password      = "Abc@123"
  charging_mode = "postPaid"
}

# 2.1 Query the available Master/Standby flavors of the specified capacity.
data "hcso_dcs_flavors" "master_standby_flavors" {
  cache_mode = "ha"
  capacity   = 0.125
}

# 2.2 Create Master/Standby Redis instances
resource "hcso_dcs_instance" "master_standby_instance" {
  engine         = "redis"
  name           = "master_standby_instance"
  engine_version = "5.0"
  capacity       = data.hcso_dcs_flavors.master_standby_flavors.capacity
  flavor         = data.hcso_dcs_flavors.master_standby_flavors.flavors[0].name

  availability_zones = [
    data.hcso_availability_zones.zones.names[0],
    data.hcso_availability_zones.zones.names[1]
  ]
  vpc_id        = hcso_vpc.vpc_1.id
  subnet_id     = hcso_vpc_subnet.vpc_subnet_1.id
  password      = "Abc@123"
...
data "hcso_dcs_flavors" "proxy_cluster_flavors" {
  cache_mode = "proxy"
  capacity   = 4
}

# 3.2 Create Proxy Cluster Redis instances
resource "hcso_dcs_instance" "proxy_cluster_instance" {
  engine         = "redis"
  name           = "proxy_cluster_instance"
  engine_version = "5.0"
  capacity       = data.hcso_dcs_flavors.proxy_cluster_flavors.capacity
  flavor         = data.hcso_dcs_flavors.proxy_cluster_flavors.flavors[0].name
...
data "hcso_dcs_flavors" "redis_cluster_flavors" {
  cache_mode = "cluster"
  capacity   = 4
}

# 4.2 Create Cluster Redis instances, and configure Backup Policy, Whitelists and Tags.
resource "hcso_dcs_instance" "redis_cluster_instance" {
  engine         = "redis"
  name           = "redis_cluster_instance"
  engine_version = "5.0"
  capacity       = data.hcso_dcs_flavors.redis_cluster_flavors.capacity
  flavor         = data.hcso_dcs_flavors.redis_cluster_flavors.flavors[0].name
